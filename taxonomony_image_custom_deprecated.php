...
class taxonomy_images_plugin {
	...
	/* CUSTOM METHOD TO JUST RETURN IMG SRC - dardoleker */
	public function get_image_src($term_tax_id = false, $size = 'medium', $icon = false ) {
		if ( false === $term_tax_id ) {
			return false;
		} else {
			$o = '';
			...
			if ( isset( $this->settings[ $term_tax_id ] ) ) {
				$attachment_id = (int) $this->settings[ $term_tax_id ];
				$o = wp_get_attachment_image_src( $attachment_id, $size, $icon );
				$o = $o[0];
			}
			return $o;
		}
